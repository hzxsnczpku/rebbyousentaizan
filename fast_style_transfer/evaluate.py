...
def from_pipe(opts):
    command = ["ffprobe", '-v', "quiet", '-print_format', 'json', '-show_streams', opts.in_path]
    ...
    command = ["ffmpeg", '-loglevel', "quiet", '-i', opts.in_path, '-f', 'image2pipe', '-pix_fmt', 'rgb24',
               '-vcodec', 'rawvideo', '-']
...
def build_parser():
    ...
    parser.add_argument('--checkpoint', type=str, dest='checkpoint_dir', required=True)
    parser.add_argument('--in-path', type=str, dest='in_path', help='dir or file to transform', required=True)
    help_out = 'destination (dir or file) of transformed file or files'
    parser.add_argument('--out-path', type=str, dest='out_path', help=help_out, required=True)
    parser.add_argument('--device', type=str, dest='device', help='device to perform compute on', default='/gpu:0')
    parser.add_argument('--batch-size', type=int, dest='batch_size', help='batch size for feedforwarding',
                        metavar='BATCH_SIZE', default=4)
    parser.add_argument('--allow-different-dimensions', action='store_true', dest='allow_different_dimensions')
    return parser
